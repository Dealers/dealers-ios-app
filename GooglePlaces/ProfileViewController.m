...
- (void)setTopPart {
    ...
    lowestYPoint = CGRectGetMaxY(self.dealerName.frame) + SECTION_GAP;
    
    if ([self.profileMode isEqualToString:@"Other Profile"]){
        
//        self.dealerProfileImage.image = [appDelegate otherProfilePic:self.dealer.photoURL];
        ...
        self.dealerProfileImage.image = [appDelegate myProfilePic];
        
        [self setSettingsButton];
        //        [self setProfileOptions];
    }
    
    self.dealerProfileImage.layer.cornerRadius = self.dealerProfileImage.frame.size.width / 2;
    self.dealerProfileImage.layer.masksToBounds = YES;
    self.dealerName.text = self.dealer.fullName;
...
{
    UIButton *settings = [appDelegate actionButton];
    ...
    [settings setBackgroundColor:[UIColor whiteColor]];
    [[settings layer] setBorderColor:[UIColor colorWithWhite:0.9 alpha:1.0].CGColor];
    [[settings layer] setBorderWidth:1.0];
    [settings setTitle:@"Settings" forState:UIControlStateNormal];
    [[settings titleLabel] setFont:[UIFont fontWithName:@"Avenir-Roman" size:17.0]];
    [settings setTitleColor:[appDelegate textGrayColor] forState:UIControlStateNormal];
    [settings setTitleColor:[UIColor groupTableViewBackgroundColor] forState:UIControlStateHighlighted];
    [settings addTarget:self action:@selector(pushSettingsView) forControlEvents:UIControlEventTouchUpInside];
    [self.scrollView addSubview:settings];
    
    UIImageView *settingsIcon = [[UIImageView alloc]initWithFrame:CGRectMake(10, 7, 20.0, 20.0)];
    settingsIcon.image = [UIImage imageNamed:@"Settings Icon"];
    [settings addSubview:settingsIcon];
    
    lowestYPoint = CGRectGetMaxY(settings.frame);
}
...
{
    UIView *profileOptionsSection = [[UIView alloc]initWithFrame:CGRectMake(0, lowestYPoint + SECTION_GAP, self.view.frame.size.width, 60)];
...
{
    self.segmentedControl = [[UISegmentedControl alloc]initWithItems:[NSArray arrayWithObjects:@"Deals", @"Likes", nil]];
    [self.segmentedControl setFrame:CGRectMake(20, lowestYPoint + SECTION_GAP, self.view.frame.size.width - 40, 30)];
    [self.segmentedControl setBackgroundColor:[UIColor whiteColor]];
    
    NSDictionary *textAttributesNormalState = @{
                                                NSFontAttributeName : [UIFont fontWithName:@"Avenir-Roman" size:17.0]
                                                };
    NSDictionary *textAttributesSelectedState = @{
                                                  NSFontAttributeName : [UIFont fontWithName:@"Avenir-Roman" size:17.0],
                                                  NSForegroundColorAttributeName : [UIColor whiteColor]
                                                  };
    
    [self.segmentedControl setTitleTextAttributes:textAttributesNormalState forState:UIControlStateNormal];
    [self.segmentedControl setTitleTextAttributes:textAttributesSelectedState forState:UIControlStateSelected];
    ...
    lowestYPoint = CGRectGetMaxY(self.segmentedControl.frame);
}

- (void)setDealsView
{
    self.dealsView = [[UIView alloc]initWithFrame:CGRectMake(0, lowestYPoint + 10, self.view.frame.size.width, 300)];
    self.dealsView.backgroundColor = [UIColor blackColor];
    [self.scrollView addSubview:self.dealsView];
}

- (void)setLikesView
{
    self.likesView = [[UIView alloc]initWithFrame:self.dealsView.frame];
...
- (void)segmentedControlAction:(id)sender
{
    ...
    if (selectedSegment == 0) {
        ...
        [UIView animateWithDuration:0.3
                         animations:^{
                             self.dealsView.alpha = 0;
                         }
                         completion:^(BOOL finished) {
                             self.dealsView.hidden = YES;
                             self.likesView.hidden = NO;
                             [UIView animateWithDuration:0.3
                                              animations:^{
                                                  self.likesView.alpha = 1.0;
                                              }];
                         }];
    }
}
