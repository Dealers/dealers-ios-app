...
-(void) selectDealButtonForLikeViewClicked:(id)sender {
    ...
    if (![[dealClass photoID1] isEqualToString:@"0"]) {
        controller.deal.photo1 = [_photoidConvertedArrayForLikesView objectAtIndex:(button.tag-1)];
...
-(void) selectDealButtonClicked:(id)sender {
    ...
    if (![[dealClass photoID1] isEqualToString:@"0"]) {
        controller.deal.photo1 = [_PHOTOIDMARRAYCONVERT objectAtIndex:(button.tag)];
        controller.isShortCell = @"no";
    } else controller.isShortCell = @"yes";
...
-(void) setTopPart {
    ...
    if ([self.profileMode isEqualToString:@"Other Profile"]){
        ...
        self.currentDealer = self.appDelegate.dealer;
...
- (void)determineProfileMode
{
    ...
    } else if ([self.appDelegate.dealer.url isEqualToString:self.dealerId]) {
